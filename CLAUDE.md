...
# MAC 연산 기반 패턴 매칭 (난이도 1, 11개 테스트)
python3 -m pytest entry/cs/mission01/tests/ -v

# 2D 컨볼루션 기반 특징 추출 (난이도 1, 12개 테스트)
python3 -m pytest entry/cs/mission02/tests/ -v
...
│   ├── cs/                                  #   [컴퓨터과학] 컴퓨터과학 기초
│       ├── mission01/                      #     MAC 연산 기반 패턴 매칭
│       │   ├── problem.md, solution.md
│       │   ├── template/                   #       mac_scorer.py (스켈레톤)
│       │   ├── sample_submission/          #       mac_scorer.py (정답)
│       │   └── tests/
│       │       ├── conftest.py
│       │       └── test_mac_scorer.py      #       11개 테스트
│       └── mission02/                      #     2D 컨볼루션 기반 특징 추출
│           ├── problem.md, solution.md
│           ├── template/                   #       conv2d_analyzer.py (스켈레톤)
│           ├── sample_submission/          #       conv2d_analyzer.py (정답)
│           └── tests/
│               ├── conftest.py
...
### AST 분석 + subprocess 복합 미션 처리

python_mission01, ds_mission01, algo_mission01은 AST 분석과 subprocess 실행을 복합 사용하며, 모듈 수준 글로벌 변수 패턴을 사용.
...
- **python_mission01**: `_import_module("book_manager")`로 import + `book_manager.py` subprocess 실행
- **ds_mission01**: `lru_cache.py` AST 분석 + subprocess REPL
- **algo_mission01**: `mini_git.py` AST 분석 + subprocess REPL
- **cs_mission01**: `mac_scorer.py` AST 분석 + `importlib` 모듈 import
- **cs_mission02**: `conv2d_analyzer.py` AST 분석 + `importlib` 모듈 import
...
#### entry_python_mission01 - Git 브랜치 워크플로우
...
| 테스트 (총 4개) | AI 트랩 |
|-----------------|--------|
| q1_commit, q2_branch, q3_checkout, q4_merge | 없음 |

---

#### entry_python_mission02 - 미니 펫 시뮬레이터
...
#### entry_python_mission03 - Git 워크플로우 시뮬레이터

- **난이도**: 1 | **권장 시간**: 15분 | **합격**: 100점 (10개 전체)
- **제출물**: git_simulator.py (1파일)
- **정답 예시**: `entry/python/mission03/sample_submission/`
- **검증 방식**: AST 구조 분석 + `importlib`로 모듈 import 후 기능 검증
...
#### entry_cs_mission01 - MAC 연산 기반 패턴 매칭

- **난이도**: 1 | **권장 시간**: 15분 | **합격**: 100점 (11개 전체)
- **제출물**: mac_scorer.py (1파일)
- **정답 예시**: `entry/cs/mission01/sample_submission/`
- **검증 방식**: AST 구조 분석 + `importlib`로 모듈 import 후 기능 검증

| 테스트 클래스 | 테스트 (총 11개) | AI 트랩 |
|-------------|-----------------|--------|
| `TestStructure` | functions_exist, no_external_lib | |
| `TestMAC` | mac_basic, mac_floats, find_best_match | |
| `TestNormalize` | normalize_labels, is_close | |
| `TestBenchmark` | measure_mac_time, analyze_complexity | |
| `TestDiagnosis` | diagnose_failure | |
| `TestMain` | main_result | |

---

#### entry_cs_mission02 - 2D 컨볼루션 기반 특징 추출

- **난이도**: 1 | **권장 시간**: 15분 | **합격**: 100점 (12개 전체)
- **제출물**: conv2d_analyzer.py (1파일)
- **정답 예시**: `entry/cs/mission02/sample_submission/`
- **검증 방식**: AST 구조 분석 + `importlib`로 모듈 import 후 기능 검증

| 테스트 클래스 | 테스트 (총 12개) | AI 트랩 |
|-------------|-----------------|--------|
| `TestStructure` | functions_exist, no_external_lib | |
| `TestConv2d` | pad_matrix, conv2d_basic, conv2d_output_size | |
| `TestActivation` | relu, flatten | |
| `TestStats` | compute_stats | |
| `TestFeatures` | extract_features, find_strongest_feature | |
| `TestMain` | main_feature_sums, main_strongest | |

---

#### entry_python_mission04 - 프롬프트 관리 프로그램

- **난이도**: 1 | **권장 시간**: 15분 | **합격**: 70점
- **제출물**: prompt_manager.py (1파일)
- **정답 예시**: `entry/python/mission04/sample_submission/`
- **검증 방식**: AST 구조 분석 + subprocess REPL 기능 검증
...
| 구분 | 수치 |
|------|------|
| 총 미션 수 | 16개 (입학연수 10, Linux 1, Python 2, DS 1, Algo 1, DB 1) |
| 총 테스트 수 | 160개 |
| 총 AI 트랩 항목 | 27개 |
...
| `entry_cs_mission01` | 입학연수 | 1 | 15분 | 11 | 0 | mac_scorer.py |
| `entry_cs_mission02` | 입학연수 | 1 | 15분 | 12 | 0 | conv2d_analyzer.py |
