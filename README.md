...
| 패턴 | 설명 | 사용 미션 |
|------|------|---------|
| **AST 분석형** | `ast` 모듈로 소스코드 구문 분석 | Python M1, DS, Algo, CS |
| **importlib import형** | `importlib`로 모듈 동적 로드 후 기능 검증 | CS, entry Python M2/M3 |
| **subprocess 실행형** | `subprocess.run()`으로 학생 코드 실행 후 출력 검증 | Python, Linux, DS, Algo, DB, entry Python M4 |
