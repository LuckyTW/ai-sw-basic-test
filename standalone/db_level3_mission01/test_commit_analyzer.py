"""
db_level3_mission01 — 커밋 이력 DB 분석기 standalone pytest

원본 Validator:
  - plugins/db/validators/schema_validator.py (5개 CheckItem)
  - plugins/db/validators/analysis_validator.py (8개 CheckItem)
  - plugins/db/validators/report_validator.py (6개 CheckItem)
...
def db_and_report(tmp_path_factory):
...
# ---------------------------------------------------------------------------
# 섹션 추출 헬퍼 (AnalysisValidator._find_section_lines 재현)
# ---------------------------------------------------------------------------
...
class TestSchema:
    ...
    def test_root_commit_null(self, db_and_report):
        ...
        AI 트랩: parent_hash를 NOT NULL로 정의하면 root commit INSERT 실패.
        정답: parent_hash는 NULL 허용, root commit(최초 커밋)은 parent가 없으므로 NULL.
        """
        db_path, _ = db_and_report
        conn = sqlite3.connect(db_path)
        try:
...
class TestAnalysis:
    ...
    def test_author_left_join(self, db_and_report):
        ...
        AI 트랩: INNER JOIN으로 커밋 없는 작성자(황서진) 제외.
        정답: LEFT JOIN으로 0-commit 작성자도 포함.
        """
        _, report_text = db_and_report
        lines = report_text.splitlines()
    ...
    def test_branch_no_commits(self, db_and_report):
        ...
        AI 트랩: INNER JOIN으로 커밋 없는 브랜치(hotfix/urgent) 제외.
        정답: LEFT JOIN으로 0-commit 브랜치도 포함.
        """
        _, report_text = db_and_report
        lines = report_text.splitlines()
        found = any(
    ...
    def test_distinct_file_count(self, db_and_report):
        ...
        AI 트랩: COUNT(*)로 중복 파일을 별도 카운트.
        정답: COUNT(DISTINCT file_path)로 고유 파일 수 산출.
        """
        _, report_text = db_and_report
        lines = report_text.splitlines()
    ...
    def test_self_join_parent(self, db_and_report):
        ...
        AI 트랩: INNER JOIN self-reference로 parent=NULL인 root commit 제외.
        정답: Recursive CTE 또는 LEFT JOIN으로 root commit 포함.
        """
        _, report_text = db_and_report
