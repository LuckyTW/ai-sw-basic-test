...
ACCEPTED = {
    "q1": ["git init"],
    "q2": ["git add .", "git add -A", "git add --all"],
    "q3": ["git status"],
    "q4": ["git log", "git log --oneline"],
}
...
class TestGitBasic:
    ...
    def test_q1_init(self, answers):
        """새 Git 저장소 초기화 (25점)"""
        _check(answers, "q1", ACCEPTED["q1"])

    def test_q2_add(self, answers):
        """모든 변경 파일 스테이징 (25점)"""
        _check(answers, "q2", ACCEPTED["q2"])

    def test_q3_status(self, answers):
        """저장소 상태 확인 (25점)"""
        _check(answers, "q3", ACCEPTED["q3"])

    def test_q4_log(self, answers):
        """커밋 이력 확인 (25점)"""
