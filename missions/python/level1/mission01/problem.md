...
PBL 과정에서 배운 개념(dataclass, 제너레이터, 데코레이터, 파일 I/O, CLI)을 **도서 관리 시스템**에 적용합니다.

배포된 `template/book_manager.py` 스켈레톤 파일의 `TODO` 부분을 구현하세요.

| 역할 | 배점 |
|------|------|
| Book 데이터 모델 정의 | 25점 |
| 검색/필터링 (제너레이터, 데코레이터) | 25점 |
| 파일 저장/로드 | 20점 |
| CLI 인터페이스 | 30점 |
...
- **book_manager.py 존재** (5점): 파일이 있어야 합니다
- **--help 옵션** (5점):
  - `python book_manager.py --help` 실행 시 도움말 출력
  - ❌ argparse 없이 구현하면 **0점**
- **add 서브커맨드** (8점):
  - `python book_manager.py add --isbn "978-..." --title "제목" --author "저자" --price 10000`
- **list 서브커맨드** (7점):
