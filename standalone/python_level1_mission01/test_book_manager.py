"""
Python 도서 관리 시스템 — standalone pytest 검증 (17개 테스트)

원본 Validator 4개(ModelValidator, PatternValidator, CLIValidator, PersistenceValidator)의
...
SUBMISSION_FILE = "book_manager.py"
...
def _import_submission():
    """제출 모듈을 안전하게 import (캐시 제거 후 재로딩)"""
    resolved = str(os.path.abspath(SUBMISSION_DIR))
...
def _run_cli(args: list, timeout: int = 10, cwd: Optional[str] = None) -> Optional[subprocess.CompletedProcess]:
    """합본 book_manager.py를 subprocess로 실행"""
    work_dir = cwd or SUBMISSION_DIR
    try:
        return subprocess.run(
            [sys.executable, SUBMISSION_PATH] + args,
            capture_output=True,
            text=True,
            timeout=timeout,
            cwd=work_dir,
...
class TestModelValidator:
    ...
    def test_model_dataclass(self):
        ...
        # 런타임 확인
        stu = _import_submission()
        book_cls = getattr(stu, "Book", None)
        assert book_cls is not None, "Book 클래스를 찾을 수 없습니다"
...
class TestPatternValidator:
    ...
    def test_pattern_yield(self):
        ...
        stu = _import_submission()
        search_fn = getattr(stu, "search_books", None)
...
class TestCLIValidator:
    ...
    def test_cli_runnable(self):
        ...
        assert os.path.isfile(SUBMISSION_PATH), f"{SUBMISSION_FILE}이 존재하지 않습니다"
    ...
    def test_cli_add(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            # 제출 파일을 임시 디렉토리에 복사
            shutil.copy2(SUBMISSION_PATH, tmpdir)
    ...
    def test_cli_no_crash(self):
        ...
        if result is None:
            assert not os.path.isfile(SUBMISSION_PATH), "실행에 실패했지만 파일은 존재합니다"
            return
...
class TestPersistenceValidator:
    ...
    @staticmethod
    def _do_save(stu, books, tmpdir):
        """save_books 호출, 생성된 파일 경로 반환"""
        save_fn = getattr(stu, "save_books", None)
        if save_fn is None:
            return None
    ...
    def test_persist_roundtrip(self):
        ...
        stu = _import_submission()
        books = self._make_test_books(stu)
        assert books is not None, "테스트용 Book 인스턴스를 생성할 수 없습니다"

        with tempfile.TemporaryDirectory() as tmpdir:
            saved_file = self._do_save(stu, books, tmpdir)
            assert saved_file is not None, "save_books가 파일을 생성하지 못했습니다"

            load_fn = getattr(stu, "load_books", None)
    ...
    def test_persist_no_pickle(self):
        ...
        for pattern in ["*.pkl", "*.pickle"]:
            found = gl.glob(os.path.join(SUBMISSION_DIR, "**", pattern), recursive=True)
